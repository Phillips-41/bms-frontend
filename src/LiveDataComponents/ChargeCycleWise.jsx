...
function ChargeCycleWise() {
  const theme = useTheme();
  const colors = tokens(theme.palette.mode);
 const { data } = useContext(AppContext);
  const device = data[0];
  const { chargeTimeCycle, systemPeakCurrentInChargeOneCycle } = device;
  ...
  return (
    ...
    >
      ...
      <Box display="flex" flexDirection="column" width="100%">
        {[
         { label: 'Peak Current', value: systemPeakCurrentInChargeOneCycle, unit: 'A', icon: 'peakChargeIcon' },
         { label: 'Run Time', value: ChargeTime ? ChargeTime(chargeTimeCycle) : chargeTimeCycle, unit: '', icon: 'chargeTimeIcon' },
